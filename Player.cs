...
public class PlayerController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (TryActivateSignUnderCrosshair())
        {
            // sign activated
        }
    ...
    private bool TryActivateSignUnderCrosshair()
    {
        if (!Input.GetKeyDown(KeyCode.E)) return false;

        Camera cam = (cameraTransform != null) ? cameraTransform.GetComponentInParent<Camera>() ?? Camera.main : Camera.main;
        if (cam == null)
        {
            if (interactionVerboseLogs) Debug.LogWarning("PlayerController: No camera for sign raycast.");
            return false;
        }

        if (interactionVerboseLogs) Debug.DrawRay(cam.transform.position, cam.transform.forward * raycastDistance, Color.cyan, 1f);

        Ray ray = new Ray(cam.transform.position, cam.transform.forward);
        if (Physics.Raycast(ray, out RaycastHit hit, raycastDistance, raycastLayerMask))
        {
            Sign signComp = hit.collider.GetComponent<Sign>() ?? hit.collider.GetComponentInParent<Sign>();
            if (signComp != null)
            {
                if (interactionVerboseLogs) Debug.Log($"Activating Sign '{signComp.name}'");
                signComp.ActivateSign();
                return true;
            }
        }
        return false;
